...
namespace Client
{
    public partial class AIPlayForm : Form
    {
        ...
        private int limit = 1;  // Depth. 몇 수 앞을 내다 볼 것인지.
        ...
        /**
         * 기본적으로 각 방향으로 4목이 생성되면 count에 2를 설정함.
         * 이후에 한 쪽이라도 막혀있으면 count를 1씩 뺀다.
         * 열린 4은 두 곳 모두 열려있으므로 2를 반환.
         * 하나 닫힌 4은 1을 반환하며 닫힌 4은 0을 반환
         * 5목이 만들어진 경우에서 return 하므로 4목 중 5목에 해당하는 경우는 따로 체크할 필요가 없다.
         * 4목이 만들어지지 않는 경우 -1 리턴.
         * 총 16가지
         */
        ...
        public int make4mok4(int x, int y)
        {
            ...
            try
            {
                if (board[x, y + 3] == 0 && board[x, y] == board[x, y + 1] && board[x, y] == board[x, y + 2] && board[x, y] == board[x, y + 4])
                    count = 1;
            }
            catch (IndexOutOfRangeException)
            {
        ...
        /**
         * 기본적으로 각 방향으로 3목이 생성되면 count에 2를 설정함.
         * 이후에 한 쪽이라도 막혀있으면 count를 1씩 뺀다.
         * 열린 3은 두 곳 모두 열려있으므로 2를 반환.
         * 하나 닫힌 3은 1을 반환하며 닫힌 3은 0을 반환
         * 3목에 해당하지 않는 경우 -1을 반환.
         */
        // 총 12개
        ...
        /**
         * 기본적으로 각 방향으로 2목이 생성되면 count에 2를 설정함.
         * 이후에 한 쪽이라도 막혀있으면 count를 1씩 뺀다.
         * 열린 2은 두 곳 모두 열려있으므로 2를 반환.
         * 하나 닫힌 2은 1을 반환하며 닫힌 2은 0을 반환
         * 2목에 해당하지 않는 경우 -1을 반환.
         */
        // 총 4개
        ...
        public int make2mok1(int x, int y)
        {
            ...
            try
            {
                if (board[x, y] == board[x, y + 1])
                {
                    count = 2;
                    // 3목인 경우
                    if (y < edgeCount - 2 && board[x, y] == board[x, y + 2])
                        return -1;
                    if (y > 0 && board[x, y] == board[x, y - 1])
                        return -1;
                    // 3목이 아닌 경우 닫혔는지 확인
                    if (y == edgeCount - 2 || board[x, y + 2] != 0)
                        count--;
        ...
        public int make2mok2(int x, int y)
        {
            ...
            try
            {
                if (board[x, y] == board[x - 1, y + 1])
                {
                    count = 2;
                    // 3목인 경우
                    if (x > 1 && y < edgeCount - 2 && board[x, y] == board[x - 2, y + 2])
                        return -1;
                    if (x < edgeCount - 1 && y > 0 && board[x, y] == board[x + 1, y - 1])
                        return -1;
                    // 3목이 아닌 경우 닫혔는지 확인
                    if (x == 1 || y == edgeCount - 2 || board[x - 2, y + 2] != 0)
                        count--;
                    if (x == edgeCount - 1 || y == 0 || board[x + 1, y - 1] != 0)
                        count--;
                }
            }
            catch (IndexOutOfRangeException)
        ...
        // AlphaBetaPruning 알고리즘 함수; DFS 기반.
        // alpha: 최대한 큰 값을 선택 해야 함(Max 역할). beta: 최대한 작은 값을 선택해야 함.(Min 역할)
        // level: 0부터 시작. 루트 노드부터 시작.
        int AlphaBetaPruning(int level, int alpha, int beta)
        {
            // 현재 최대 깊이인 limit에 도달한다면
            if (level == limit)
            {
                // AI의 평가 가치에서 플레이어의 평가 가치를 뺀 수를 반환. (플레이어의 수의 가치를 평가.)
                ...
                for (int i = 0; i < edgeCount; i++)
                {
                    for (int j = 0; j < edgeCount; j++)
                    {
                        if (board[i, j] == 0)
                        {
                            ...
                            {
                                beta = min;
                                // 만약에 현재 알파 값이 베타 값보다 크다면 더이상 노드를 볼 필요가 없음.
                                if (alpha >= beta) find = 1;
                            }
                        }
                        if (find == 1) break;
